...
public class Quadtree<T> where T : IQuadtreeStorable
{
    ...
    public bool Divided => divided; // (新增) 公共访问器，用于调试绘制

    // --- 子节点引用 ---

    // (已修改为泛型)
    private Quadtree<T> northwest;
    public Quadtree<T> Northwest => northwest; // (新增) 公共访问器

    private Quadtree<T> northeast;
    public Quadtree<T> Northeast => northeast; // (新增) 公共访问器

    private Quadtree<T> southwest;
    public Quadtree<T> Southwest => southwest; // (新增) 公共访问器

    private Quadtree<T> southeast;
    public Quadtree<T> Southeast => southeast; // (新增) 公共访问器
