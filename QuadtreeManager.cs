...
public class QuadtreeManager
{
    ...
    /// </summary>
    private readonly List<QuadtreeObjectWrapper> registeredObjects = new();
 
    /// <summary>
    /// 用于缓存查询结果的列表。(已修改为存储包装类)
    /// </summary>
    private readonly List<QuadtreeObjectWrapper> _queryResultCache = new();

    /// <summary>
    /// QuadtreeObjectWrapper的对象池，用于避免运行时GC。
    /// </summary>
    private readonly Stack<QuadtreeObjectWrapper> _wrapperPool = new(1000); // 初始容量设为1000
